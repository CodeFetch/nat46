...
int xlate_mac_to_v4(nat46_instance_t *nat46, nat46_xlate_rule_t *rule, void *pipv6, void *pipv4) {
  ...
  int cmp = -1;
  int v6_pref_len = rule->v6_pref_len;
  int v4_pref_len = rule->v6_pref_len;
...
int pairs_xlate_v4_to_v6_outer(nat46_instance_t *nat46, struct ethhdr *pmac, struct iphdr *hdr4, uint16_t *sport, uint16_t *dport, void *v6saddr, void *v6daddr) {
  int ipair = 0;
  nat46_xlate_rulepair_t *apair = NULL;
  int xlate_src = -1;
  int xlate_dst = -1;
  ...
  for(ipair = 0; ipair < nat46->npairs; ipair++) {
    ...
    if(-1 == xlate_src) {
      if(xlate_v4_to_v6(nat46, &apair->local, &pmac->h_source, &hdr4->saddr, v6saddr, sport)) {
        xlate_src = ipair;
      }
    }
    if(-1 == xlate_dst) {
      if(xlate_v4_to_v6(nat46, &apair->remote, &pmac->h_dest, &hdr4->daddr, v6daddr, dport)) {
        xlate_dst = ipair;
      }
    }
    if( (xlate_src >= 0) && (xlate_dst >= 0) ) {
...
void nat46_ipv4_input(struct sk_buff *old_skb) {
  ...
  struct ethhdr *mac;
  mac = (struct ethhdr*)skb_mac_header(old_skb);
  ...
  if(!pairs_xlate_v4_to_v6_outer(nat46, mac, hdr4, having_l4 ? &sport : NULL, having_l4 ? &dport : NULL, v6saddr, v6daddr)) {
