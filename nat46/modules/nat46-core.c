...
nat46debug_dump(nat46_instance_t * nat46, int level, void * addr, int len) {
  ...
  while (--len >= 0) {
    if (i % 16 == 0) {
      ...
      pc1 = buf1;
      pc2 = buf2;
    } * pc1++ = tohex[ * pc >> 4]; * pc1++ = tohex[ * pc & 15]; * pc1++ = ' ';

    if ( * pc >= 32 && * pc < 127) { * pc2++ = * pc;
    } else { * pc2++ = '.';
    }
    i++;
    pc++;
    if (i % 16 == 0) { * pc1 = 0; * pc2 = 0;
      nat46debug(level, "%s:   %s  %s", buf0, buf1, buf2);
    }

  }
  if (i % 16 != 0) {
    while (i % 16 != 0) { * pc1++ = ' '; * pc1++ = ' '; * pc1++ = ' '; * pc2++ = ' ';
      i++;
    } * pc1 = 0; * pc2 = 0;
    nat46debug(level, "%s:   %s  %s", buf0, buf1, buf2);
  }
...
int try_parse_ipv6_prefix(struct in6_addr * pref, int * pref_len, char * arg) {
  ...
  if (arg_plen) { * arg_plen++ = 0;
    if (pref_len) { * pref_len = simple_strtol(arg_plen, NULL, 10);
    }
  }
...
int try_parse_rule_arg(nat46_xlate_rule_t * rule, char * arg_name, char * * ptail) {
  ...
  if (NULL == val) {
    ...
    } else if (0 == strcmp("MAC", val)) {
      rule - > style = NAT46_XLATE_MAC;
    } else {
      err = 1;
    }
  }
...
int nat46_get_ipair_config(nat46_instance_t * nat46, int ipair, char * buf, int count) {
  ...
  ret = snprintf(buf, count, format, & apair - > local.v4_pref, apair - > local.v4_pref_len, & apair - > local.v6_pref, apair - > local.v6_pref_len,
    xlate_style_to_string(apair - > local.style),
    apair - > local.ea_len, apair - > local.psid_offset,

    & apair - > remote.v4_pref, apair - > remote.v4_pref_len, & apair - > remote.v6_pref, apair - > remote.v6_pref_len,
...
void xlate_v4_to_mac(nat46_instance_t * nat46, nat46_xlate_rule_t * rule, unsigned char * mac, void * pipv6) {
  char * ipv6 = pipv6;

  /* 'u' byte and suffix are zero */
  memset( & ipv6[8], 0, 8);
  if (rule - > v6_pref_len == 80) {
    memcpy(ipv6, & rule - > v6_pref, 10);
    memcpy( & ipv6[11], mac, 6);
    break;
  }
}
...
int xlate_mac_to_v4(nat46_instance_t * nat46, nat46_xlate_rule_t * rule, void * pipv6, void * pipv4) {
  char * ipv4 = pipv4;
  char * ipv6 = pipv6;
  int cmp = -1;
...
#
define CHAR_BIT 8# define PREPARE_FIRST_COPY()\
do {\
  if (src_len >= (CHAR_BIT - dst_offset_modulo)) {\ * dst &= reverse_mask[dst_offset_modulo];\
    src_len -= CHAR_BIT - dst_offset_modulo;\
  } else {\ * dst &= reverse_mask[dst_offset_modulo]\ | reverse_mask_xor[dst_offset_modulo + src_len + 1];\
...
bitarray_copy(const void * src_org, int src_offset, int src_len,
  ...
  */
  static
  const unsigned char reverse_mask[] = {
    0x55,
    0x80,
    0xc0,
    0xe0,
  ...
  static
  const unsigned char reverse_mask_xor[] = {
    0xff,
    0x7f,
    0x3f,
    0x1f,
  ...
  if (src_len) {
    ...
    if (src_offset_modulo == dst_offset_modulo) {
      ...
      if (src_len_modulo) { * dst &= reverse_mask_xor[src_len_modulo]; * dst |= reverse_mask[src_len_modulo] & * src;
      }
    } else {
      int bit_diff_ls,
      bit_diff_rs;
      ...
      if (src_len_modulo) {
        ...
        * dst &= reverse_mask_xor[src_len_modulo]; * dst |= c;
      }
    }
  }
}
...
int xlate_v4_to_v6(nat46_instance_t * nat46, nat46_xlate_rule_t * rule, unsigned char * mac, void * pipv4, void * pipv6, uint16_t * pl4id) {
  ...
  switch (rule - > style) {
  case NAT46_XLATE_NONE:
    /* always fail unless it is a host 1:1 translation */
  ...
  case NAT46_XLATE_RFC6052:
    ...
    break;
  case NAT46_XLATE_MAC:
    ret = xlate_v4_to_mac(nat46, rule, mac, pipv6);
    break;
  }
  return ret;
}
...
int xlate_v6_to_v4(nat46_instance_t * nat46, nat46_xlate_rule_t * rule, void * pipv6, void * pipv4) {
  ...
  case NAT46_XLATE_RFC6052:
    ...
    break;
  case NAT46_XLATE_MAC:
    ret = xlate_mac_to_v4(nat46, rule, pipv6, pipv4);
    break;
  }
  return ret;
}
...
__sum16 csum_ipv6_unmagic(nat46_instance_t * nat46,
...
void fill_v4hdr_from_v6hdr(struct iphdr * iph, struct ipv6hdr * ip6h, __u32 v4saddr, __u32 v4daddr, __u16 id, __u16 frag_off, __u16 proto, int l3_payload_len) {
  ...
  iph - > saddr = v4saddr;
  iph - > daddr = v4daddr;
  iph - > protocol = proto; * ((__be16 * ) iph) = htons((4 << 12) | (5 << 8) | (0x00 /*tos*/ & 0xff));
...
int xlate_payload6_to4(nat46_instance_t * nat46, void * pv6, void * ptrans_hdr, int v6_len, u16 * ul_sum, int * ptailTruncSize) {
  ...
  if (proto == NEXTHDR_FRAGMENT) {
    ...
    if (fh - > frag_off == 0) {
      ...
      v6_len -= 8;
      infrag_payload_len -= 8; * ptailTruncSize += 8;
      ipflags = 0;
    }
  }
  ...
  case NEXTHDR_TCP:
    {
      ...
      if (ul_sum) { * ul_sum = csum16_upd( * ul_sum, th - > check, sum2);
      }
      th - > check = sum2;
      break;
    }
  case NEXTHDR_UDP:
    {
      ...
      if (ul_sum) { * ul_sum = csum16_upd( * ul_sum, udp - > check, sum2);
      }
      udp - > check = sum2;
      break;
    }
  case NEXTHDR_ICMP:
    {
      ...
      if (ul_sum) { * ul_sum = csum16_upd( * ul_sum, sum0, sum1);
      }
  ...
  if (ul_sum) { * ul_sum = unchecksum16(pv6, (((u8 * ) ptrans_hdr) - ((u8 * ) pv6)) / 2, * ul_sum); * ul_sum = rechecksum16(iph, 10, * ul_sum);
  }

  memmove(((char * ) pv6) + IPV4HDRSIZE, get_next_header_ptr6(ip6h, v6_len), v6_len - IPV4HDRSIZE);
  memcpy(pv6, iph, IPV4HDRSIZE); * ptailTruncSize += IPV6V4HDRDELTA;
...
static void nat46_fixup_icmp6_pkt_toobig(nat46_instance_t * nat46, struct ipv6hdr * ip6h, struct icmp6hdr * icmp6h, struct sk_buff * old_skb, int * ptailTruncSize) {
  ...
  if (ntohs( * pmtu) > IPV6V4HDRDELTA) {
    icmp6h - > icmp6_cksum = csum16_upd(old_csum, * pmtu, htons(ntohs( * pmtu) - IPV6V4HDRDELTA)); * pmtu = htons(ntohs( * pmtu) - IPV6V4HDRDELTA);
  }
...
static void nat46_fixup_icmp6_paramprob(nat46_instance_t * nat46, struct ipv6hdr * ip6h, struct icmp6hdr * icmp6h, struct sk_buff * old_skb) {
  ...
  case 0:
    if ( * pptr6 < sizeof(ptr6_4) / sizeof(ptr6_4[0])) {
      ...
      if (new_pptr >= 0) {
        icmp6h - > icmp6_cksum = csum16_upd(icmp6h - > icmp6_cksum, ( * pptr6 & 0xffff), (new_pptr << 8)); * pptr4 = 0xff & new_pptr;
      } else {
        ip6h - > nexthdr = NEXTHDR_NONE;
      }
    } else {
...
static uint16_t nat46_fixup_icmp_dest_unreach(nat46_instance_t * nat46, struct iphdr * iph, struct icmphdr * icmph, struct sk_buff * old_skb) {
  ...
  case 4:
    ...
    icmph - > type = 2;
    icmph - > code = 0;
    if (ntohs( * pmtu) < 1280) { * pmtu = htons(1280);
    }
    break;
  case 5:
  case 6:
  ...
  if (xlate_dst >= 0) {
    if (xlate_src < 0) {
      if (proto == NEXTHDR_ICMP) {
        nat46debug(1, "[nat46] Could not translate remote address v6->v4, ipair %d, for ICMP6 use dest addr", ipair); * pv4saddr = * pv4daddr;
        xlate_src = xlate_dst;
      } else {
...
void nat46_ipv4_input(struct sk_buff * old_skb) {
  ...
  /* build IPv6 header */
  tclass = 0; /* traffic class */ * (__be32 * ) hdr6 = htonl(0x60000000 | (tclass << 20)) | flowlabel; /* version, priority, flowlabel */
