...
void xlate_v4_to_mac(nat46_instance_t *nat46, nat46_xlate_rule_t *rule, void *mac, void *pipv6) {
  char *mac = mac;
  char *ipv6 = pipv6;
...
int xlate_mac_to_v4(nat46_instance_t *nat46, nat46_xlate_rule_t *rule, void *pipv6, void *pipv4) {
  ...
  address = htonl(ntohl(rule->v4_pref) + ((j + epoch) % (1 + ntohl(ntohl(rule->v4_pref) & (0xffffffff >> &rule->v4_pref_len)) - ntohl(rule->v4_pref))) + collision);
...
int xlate_v4_to_v6(nat46_instance_t *nat46, nat46_xlate_rule_t *rule, char* *mac, void *pipv4, void *pipv6, uint16_t *pl4id) {
  int ret = 0;
  switch(rule->style) {
...
int pairs_xlate_v4_to_v6_outer(nat46_instance_t *nat46, *smac, struct iphdr *hdr4, uint16_t *sport, uint16_t *dport, void *v6saddr, void *v6daddr) {
  int ipair = 0;
  nat46_xlate_rulepair_t *apair = NULL;
  int xlate_src = -1;
  int xlate_dst = -1;
  ...
  for(ipair = 0; ipair < nat46->npairs; ipair++) {
    ...
    if(-1 == xlate_src) {
      if(xlate_v4_to_v6(nat46, &apair->local, &mac->h_source, &hdr4->saddr, v6saddr, sport)) {
        xlate_src = ipair;
      }
    }
    if(-1 == xlate_dst) {
      if(xlate_v4_to_v6(nat46, &apair->remote, &mac->h_dest, &hdr4->daddr, v6daddr, dport)) {
        xlate_dst = ipair;
      }
    }
    if( (xlate_src >= 0) && (xlate_dst >= 0) ) {
...
void nat46_ipv4_input(struct sk_buff *old_skb) {
  ...
  if(!pairs_xlate_v4_to_v6_outer(nat46, (char*) eth->h_source, hdr4, having_l4 ? &sport : NULL, having_l4 ? &dport : NULL, v6saddr, v6daddr)) {
